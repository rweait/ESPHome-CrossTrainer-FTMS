...
### Выбор дисплея
Дисплей 4.3 дюйма — оптимальный размер: не слишком маленький и не громоздкий, удобен для выбора программ и мониторинга данных (скорость, время, расстояние). Дополнительные данные можно вывести на внешний дисплей или планшет. Все перечисленные дисплеи были протестированы для выбора оптимального решения.

- **Nextion TN дисплей (2.8")** (~2000 руб.):
  - **Плюсы**: Компактный, недорогой.
  - **Минусы**:
    1. Очень маленький угол обзора из-за TN-матрицы.
    2. Слишком маленький размер для удобного взаимодействия.
  - **Вывод**: Не выбран из-за ограниченного угла обзора и маленького размера. Дисплей 3.2" с TN-матрицей также не оптимален из-за тех же проблем с углами обзора.

- **IPS LVGL дисплей (Guition JC4827W543, 4.3")** (~1200 руб., без доставки):
  - **Плюсы**: Низкая цена, IPS-панель, ёмкостнsq сенсор.
  - **Минусы**:
    1. Ограниченная память 4 МБ (можно перепаять).
    2. Заметное остаточное изображение.
    3. Неидеальная работа сенсора.
    4. Несоответствие полутонов, требуется перенастройка RGB. (Решаемо)
  - **Вывод**: Не выбран из-за проблем с остаточным изображением, ненадежным сенсором и неточностью цветов. Более дорогие версии (~3000 руб.) лишены некоторых недостатков, но всё равно уступают альтернативам. Если будет найден качественный и недорогой 4.3-дюймовый дисплей на базе LVGL, прошивка может быть переписана под него.

- **Nextion IPS дисплей (NX4827P043-011C)** (~4700 руб.):
  - **Плюсы**: IPS-панель (не путать с TN), отличные углы обзора, надёжный ёмкостной сенсор, собственный процессор снижает нагрузку на ESP32, удобная настройка интерфейса в Nextion Editor.
  - **Минусы**: Высокая цена.
  - **Вывод**: Выбран за высокое качество дисплея, надёжный сенсор и упрощённую настройку интерфейса.

  <details>
  <summary><b>IPS Дисплей NEXTION NX4827P043-011C 4,3</b> (▶️ Нажмите для деталей)</summary>

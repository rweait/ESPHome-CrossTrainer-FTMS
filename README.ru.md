...
### Выбор дисплея
Дисплей 4.3 дюйма — оптимальный размер: не слишком маленький и не громоздкий, удобен для выбора программ и мониторинга данных (скорость, время, расстояние). Дополнительные данные можно вывести на внешний дисплей или планшет.
...
- **IPS LVGL дисплей (Guition JC4827W543, 4.3")** (~1200 руб., без доставки):
  ...
  - **Минусы**:
    ...
    4. Несоответствие полутонов, требуется перенастройка RGB.
...
- **Nextion IPS дисплей (NX4827P043-011C)** (~4700 руб.):
  - **Плюсы**: IPS-панель (не путать с TN), надёжный ёмкостной сенсор, собственный процессор снижает нагрузку на ESP32, удобная настройка интерфейса в Nextion Editor.
  - **Минусы**: Высокая цена.
